<script lang="ts">
</script>

<footer class="relative py-16 px-6 border-t border-white/5">
  <div class="max-w-6xl mx-auto">
    <div class="flex flex-col md:flex-row items-center justify-between gap-6">
      <div>
        <span class="text-lg font-bold">
          <span class="text-gradient">iberi22</span>
        </span>
        <p class="text-gray-600 text-sm mt-1">Systems Architect & Developer</p>
      </div>

      <div class="flex items-center gap-6 text-sm text-gray-500">
        <a href="#about" class="hover:text-gray-300 transition-colors">About</a>
        <a href="#projects" class="hover:text-gray-300 transition-colors">Projects</a>
        <a href="#blog" class="hover:text-gray-300 transition-colors">Blog</a>
        <a href="#contact" class="hover:text-gray-300 transition-colors">Contact</a>
      </div>

      <div class="flex items-center gap-4">
        <a href="https://github.com/iberi22" target="_blank" rel="noopener noreferrer" class="text-gray-600 hover:text-neon-green transition-colors">
          <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z"/></svg>
        </a>
      </div>
    </div>

    <div class="mt-10 pt-6 border-t border-white/5 text-center">
      <p class="text-gray-700 text-xs font-mono">
        <span class="text-neon-green/40">&copy;</span> {new Date().getFullYear()} <span class="text-neon-green/60">iberi22</span>
        <span class="text-gray-700 mx-2">·</span>
        Part of <span class="text-gray-600">SouthWest AI Labs</span>
        <span class="text-gray-700 mx-2">·</span>
        Built with <span class="text-neon-green/60">Astro</span> + <span class="text-neon-blue/60">Svelte</span>
      </p>
    </div>
  </div>
</footer>
